...
**Phases 1 and 2 complete (May 2026).** `@glance-apps/intents@1.0.0` published; dayGLANCE consumes it. Phase 2.5 (optional WebDAV envelope encryption) is the active piece of work and blocks Phase 3 (lastGLANCE adoption).
...
**Blocker for Phase 3.** Added after Phases 1-2 shipped to address the privacy concern that WebDAV intent files can sit on a third-party server (Koofr, Box, Hetzner) in plaintext. Self-hosted users have full control over their WebDAV; users on hosted WebDAV providers do not. Bringing intents encryption to parity with cloud sync's optional encryption closes that gap.
...
- **Envelope shape with encryption:** plaintext envelope retains `event_id`, `timestamp`, `source_app`, `source_entity_id`, `due`, plus new fields `encrypted: true` and `iv`. Encrypted payload (base64-encoded ciphertext) contains `action`, `title`, `notes`, `tags`, `priority`, `recurring`, `project`, and any other user-readable fields. The plaintext envelope is the minimum needed for consumers to filter by `source_app`, compute idempotency keys (`source_app` + `source_entity_id` + `due`), and order/GC events without bulk decryption.
...
| PR | Scope |
|---|---|
| #9 | `schemas/v1/`: extend envelope schema with optional `encrypted: true`, `iv` (base64), and `payload_ciphertext` (base64) fields. When `encrypted` is true, structural payload fields move into the encrypted blob. Validators accept both forms. |
| #10 | `crypto/`: AES-GCM encrypt/decrypt helpers. Key parameter is a derived key (consumer passes it in; package doesn't do passphrase derivation — that lives in each consumer's existing sync-encryption code). |
| #11 | `webdav/`: extend `buildEnvelope` and `parseEnvelope` to take an optional key parameter. With a key + `encrypted: true`, transparent encrypt on write / decrypt on read. Without a key, plaintext only; encountering an encrypted envelope returns a sentinel result the consumer interprets as "skip and warn." |
| #12 | Bump package to `1.1.0`; CHANGELOG entry; manual `npm publish` |

#### dayGLANCE PRs

**Pre-work before PR #14:** verify how the cloud sync code currently exposes (or hides) the derived encryption key. The crypto helpers added in package PR #10 operate on a derived key — they don't do passphrase derivation. PR #14 (emitter) needs to hand the package a derived key. Options depending on what already exists:

- If `@glance-apps/sync` exposes the derived key (or a method to get it given the passphrase), reuse directly.
...
Resolve this before starting PR #14. The same answer applies to lastGLANCE Phase 3 PR #12, so it's worth doing the structural work once.
...
| #14 | Emitter (Phase 2 PR #9): when intents encryption is on, derive key from the cloud sync passphrase and pass it to `buildEnvelope` |
| #15 | Poller (Phase 2 PR #7): pass the derived key to `parseEnvelope` when reading events; on decryption failure (encrypted event, no key, or wrong key), log to activity log and skip |
| #16 | Activity log (Phase 2 PR #10): surface decryption failures distinctly from other event categories |

**Critical-path subset for Phase 3:** package PRs #9-12 must merge before lastGLANCE can adopt encryption. dayGLANCE PRs #12-15 land in parallel with lastGLANCE Phase 3 PRs; lastGLANCE needs the package to be at `1.1.0` but does not need dayGLANCE-side encryption to be enabled by any specific user when Phase 3 ships (the encryption is opt-in per app per user).
...
| #12 | Outbound emitter (PR #3) consumes intents encryption setting: when on, derive key from the cloud sync passphrase and pass it to the package's `buildEnvelope` |
| #13 | Inbound poller (PR #7) passes the derived key to `parseEnvelope`; on decryption failure log to activity log and skip event |
